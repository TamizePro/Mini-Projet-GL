...
public class ElementModuleServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String pathInfo = request.getPathInfo();

        // Obtaining all the  modules via the connection instance
        Connection connection = MySQLConnection.getInstance();
        //La classe DAO n'est pas encore implémenté. Quand ce sera fait tu pourras revénir
        ElementModuleDAO elementModuleDAO = new ModuleDAO(connection);

        if (pathInfo == null || pathInfo.equals("/")) {
            List<Module> modules =  moduleDAO.GetAllCompte();
